...
def test_active_memory_interest_searches_follows_and_saves_without_registration(
    ...
    memory = {
        ...
        "content": "bounded autonomous personal assistants",
        "evidence_locators": [{"kind": "accepted_decision"}],
    }
    monkeypatch.setattr(
        assistant_module.PersonalAssistant,
        "active_memories",
        lambda _self, **_kwargs: [memory],
        raising=False,
    )
    ...
    assert not (assistant.identity.path / "memories").exists()
    ledger = (
...
def test_cli_open_loop_candidate_runs_with_explicit_deterministic_limits(
    ...
    root = tmp_path / "headlong"
    root.mkdir()
    _assistant(root, lambda: 0.0)
    monkeypatch.setattr(
        assistant_module.PersonalAssistant,
        "memory_candidates",
        lambda _self, **_kwargs: [
            {
                "event_id": "candidate-open-loop",
